...
if (-not $SkipBuild) {
    ...
    # Cleanup: Remove unnecessary files from node_modules to reduce deployment size
    Write-Host "`n🧹 Cleaning up node_modules to reduce deployment size..." -ForegroundColor Yellow
    $cleanupPaths = @(
        # pdf-parse test data and web files (not needed for server-side text extraction)
        "node_modules\pdf-parse\test",
        "node_modules\pdf-parse\lib\pdf.js\v1.9.426\web",
        "node_modules\pdf-parse\lib\pdf.js\v1.10.88\web",
        "node_modules\pdf-parse\lib\pdf.js\v1.10.100\web",
        "node_modules\pdf-parse\lib\pdf.js\v2.0.550\web",
        # Keep only one pdfjs version (v1.10.100 is default)
        ...
        "node_modules\@azure\**\*.ts",
        "node_modules\mssql\**\*.ts"
    )
    
    $totalSaved = 0
    foreach ($path in $cleanupPaths) {
        if (Test-Path $path) {
            $size = (Get-ChildItem $path -Recurse -File -ErrorAction SilentlyContinue | Measure-Object -Property Length -Sum -ErrorAction SilentlyContinue).Sum / 1MB
            Remove-Item $path -Recurse -Force -ErrorAction SilentlyContinue
            $totalSaved += $size
            Write-Host "   Removed: $path ($([math]::Round($size, 2)) MB)" -ForegroundColor Gray
        }
    }
    Write-Host "✅ Cleanup complete: ~$([math]::Round($totalSaved, 1)) MB saved" -ForegroundColor Green
} else {
