...
export const getMyReferralRequests = withErrorHandling(async (req: HttpRequest, context: InvocationContext): Promise<HttpResponseInit> => {
    try {
        ...
        if (isAdmin) {
            const result = await ReferralService.getMyReferralRequests(null as any, page, pageSize, true);
            return {
                status: 200,
                jsonBody: successResponse(result, 'All seeker referral requests retrieved (admin)')
            };
        }
 ...
 * POST /referral/requests/{requestId}/convert-to-open
 */
export const convertReferralToOpen = withErrorHandling(async (req: HttpRequest, context: InvocationContext): Promise<HttpResponseInit> => {
    try {
        const user = authenticate(req);
        const requestId = (req as any).params?.requestId;
        if (!requestId || !isValidGuid(requestId)) {
        ...
        return {
            status: error instanceof NotFoundError ? 404 : error instanceof ValidationError ? 400 : 500,
            jsonBody: { success: false, error: error?.message || 'Failed to convert referral request' }
        };
    }
});
