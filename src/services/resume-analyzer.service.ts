...
export interface ResumeAnalysisResult {
  ...
  tips: Array<string | { text: string; priority: 'high' | 'medium' | 'low'; category: string }>;
  strengths: string[];
  weaknesses: string[];
  ...
  /** Achievement metrics — e.g. "4/12 bullets have quantified metrics" */
  achievementMetrics?: {
    bulletsWithMetrics: number;
    totalBullets: number;
  };
  /** Weak action verbs found in resume bullets */
  weakVerbs?: string[];
  aiModel?: string;
  jobTitle?: string;
  companyName?: string;
...
Return ONLY valid JSON (no markdown, no code blocks). Use this exact structure:
{
  ...
  "categoryScores": {
    ...
    "experience": <0-100 relevance of work experience>,
    "education": <0-100 education fit for the role>,
    "keywords": <0-100 percentage of JD keywords found in resume>
  },
  ...
  "tips": [
    {"text": "<specific actionable tip>", "priority": "high|medium|low", "category": "keywords|achievements|formatting|skills|experience"}
  ],
  "achievementMetrics": {
    "bulletsWithMetrics": <number of resume bullet points that include quantified results/numbers>,
...
- weaknesses: find AT LEAST 3. Every resume has weaknesses. If you can't find any, you're not looking hard enough.`;
  ...
  private static parseAIResponse(textResponse: string): ResumeAnalysisResult {
    ...
    try {
      ...
      // Normalize tips: accept both string[] and structured object[]
      let normalizedTips: Array<string | { text: string; priority: 'high' | 'medium' | 'low'; category: string }> = [];
      if (Array.isArray(analysis.tips)) {
        normalizedTips = analysis.tips.map((tip: any) => {
          if (typeof tip === 'string') return tip;
          if (tip && typeof tip === 'object' && typeof tip.text === 'string') {
            return {
              text: tip.text,
              priority: ['high', 'medium', 'low'].includes(tip.priority) ? tip.priority : 'medium',
              category: typeof tip.category === 'string' ? tip.category : 'general',
            };
          }
      ...
      return {
        ...
        categoryScores: {
          skills: Math.min(100, Math.max(0, Number(analysis.categoryScores?.skills) || 0)),
          experience: Math.min(100, Math.max(0, Number(analysis.categoryScores?.experience) || 0)),
          education: Math.min(100, Math.max(0, Number(analysis.categoryScores?.education) || 0)),
          keywords: Math.min(100, Math.max(0, Number(analysis.categoryScores?.keywords) || 0)),
        },
        ...
        bulletFeedback: {
          weak: Array.isArray(analysis.bulletFeedback?.weak) ? analysis.bulletFeedback.weak : [],
          improved: Array.isArray(analysis.bulletFeedback?.improved) ? analysis.bulletFeedback.improved : [],
        },
        tips: normalizedTips,
        achievementMetrics: {
          bulletsWithMetrics: Math.max(0, Number(analysis.achievementMetrics?.bulletsWithMetrics) || 0),
          totalBullets: Math.max(0, Number(analysis.achievementMetrics?.totalBullets) || 0),
        },
        weakVerbs: Array.isArray(analysis.weakVerbs) ? analysis.weakVerbs.filter((v: any) => typeof v === 'string') : [],
