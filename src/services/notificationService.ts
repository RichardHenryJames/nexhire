...
const ADMIN_EMAIL = 'parimalkumar261@gmail.com';
...
export class NotificationService {
    ...
    static async notifyNewReferralRequest(data: {
        ...
        try {
            ...
            for (const referrer of referrers) {
                try {
                    // Queue the notification (always send — referral emails are critical)
                    await this.queueNotification({
                        ...
                        data: {
                            ...
                            actionUrl: `${APP_URL}/provide-referral`
                        }
                    });

                    // Also send in-app notification
                    const { default: InAppNotificationService } = await import('./inAppNotification.service');
                    await InAppNotificationService.notifyNewReferralRequest(
                        referrer.userId, data.seekerName, data.jobTitle, data.companyName, data.requestId
                    );
            ...
            try {
                await this.queueNotification({
                    ...
                    data: {
                        ...data,
                        referrerName: 'Admin',
                        referrerEmail: ADMIN_EMAIL,
                        actionUrl: `${APP_URL}/provide-referral`
                    }
                });
            } catch (adminErr: any) {
                errors.push(`Failed for admin: ${adminErr.message}`);
            }
    ...
    static async notifyReferralClaimed(data: {
        ...
        try {
            // Always send — referral emails are critical
            await this.queueNotification({
                ...
                data: {
                    ...data,
                    actionUrl: `${APP_URL}/referrals/my-requests`
                }
            });
            ...
            try {
                await this.queueNotification({
                    ...
                    data: {
                        ...data,
                        seekerEmail: ADMIN_EMAIL,
                        actionUrl: `${APP_URL}/referrals/my-requests`
                    }
                });
            } catch (_) { /* admin notify non-critical */ }
    ...
    /**
     * Notify seeker that their referral was completed (proof submitted)
     */
    static async notifyReferralCompleted(data: {
        requestId: string;
        seekerId: string;
        seekerName: string;
        seekerEmail: string;
        ...
        try {
            ...
            console.error('❌ Error notifying referral completed:', error);
            return false;
        }
    }
