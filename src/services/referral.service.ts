...
export class ReferralService {
    ...
    static async getAvailableRequests(referrerId: string, page: number = 1, pageSize: number = 20, filters?: ReferralRequestsFilter, isAdmin: boolean = false): Promise<PaginatedReferralRequests> {
        try {
            ...
            if (isAdmin) {
                whereClause = `WHERE ${statusFilter} AND rr.ParentRequestID IS NULL`;
                queryParams = [];
                paramIndex = 0;
            } else {
                whereClause = `
                    WHERE ${statusFilter}
                    AND rr.ParentRequestID IS NULL  -- Don't show child referrals
                    AND (
                        ...
                        -- BUT exclude if this referrer's company already has a child for this request
                        (rr.OpenToAnyCompany = 1 AND NOT EXISTS (
                            SELECT 1 FROM ReferralRequests child 
                            WHERE child.ParentRequestID = rr.RequestID 
                            AND child.OrganizationID = @param0
                            AND child.Status NOT IN ('Cancelled', 'Expired')
                        ))
                    )
     ...
     */
    static async getMyReferralRequests(applicantId: string, page: number = 1, pageSize: number = 20, isAdmin: boolean = false): Promise<PaginatedReferralRequests> {
        try {
            // Ensure parameters are integers
            const safePageNumber = Math.max(1, Math.floor(page) || 1);
            const safePageSize = Math.min(100, Math.max(1, Math.floor(pageSize) || 20));

            // Admin sees ALL requests from all seekers, normal user sees only their own
            const whereClause = isAdmin
                ? 'WHERE rr.ParentRequestID IS NULL'
                : 'WHERE rr.ApplicantID = @param0 AND rr.ParentRequestID IS NULL';
            const queryParams: any[] = isAdmin ? [] : [applicantId];
            ...
            const dataQuery = `
                SELECT 
                    ...
                    rp.FileType as ProofFileType,
                    rp.Description as ProofDescription,
                    -- Seeker info (useful for admin view)
                    ua.FirstName + ' ' + ua.LastName as SeekerName,
                    ua.Email as SeekerEmail
                FROM ReferralRequests rr
                -- ✅ CHANGED: LEFT JOIN instead of INNER JOIN to include external referrals
                LEFT JOIN Jobs j ON rr.JobID = j.JobID
                LEFT JOIN Organizations jo ON j.OrganizationID = jo.OrganizationID
                ...
                LEFT JOIN ReferralProofs rp ON rr.RequestID = rp.RequestID
                LEFT JOIN Users ur ON rr.AssignedReferrerID = ur.UserID
                LEFT JOIN Applicants ap ON rr.ApplicantID = ap.ApplicantID
                LEFT JOIN Users ua ON ap.UserID = ua.UserID
                ${whereClause}
                ORDER BY rr.RequestedAt DESC
                OFFSET ${offset} ROWS
                FETCH NEXT ${safePageSize} ROWS ONLY
    ...
    static async submitReferralWithProof(referrerId: string, userId: string, dto: SubmitReferralWithProofDto): Promise<ReferralRequest> {
        try {
            ...
            // ── Open-to-Any: spawn a child request instead of closing parent ──
            if (openToAnyCompany && !request.ParentRequestID) {
                // Get referrer's current company
                const referrerOrgQuery = `
                ...
                if (!referrerOrgId) {
                    throw new ValidationError('You need a current work experience to claim open-to-any referrals');
                }
                ...
                if (referrerOrgId) {
                    ...
                    if (dupCheck.recordset?.length) {
                        throw new ValidationError('A referral from this company already exists for this request');
                    }
                }
                ...
                await dbService.executeQuery(
                    `INSERT INTO ReferralRequests (
                        ...
                        OpenToAnyCompany, MinSalary, SalaryCurrency, SalaryPeriod, PreferredLocations, ParentRequestID
                    )
                    SELECT @param0, NULL, @param5, ApplicantID, ResumeID, 'Completed', RequestedAt, ExpiryTime,
                        @param1, @param2, @param3, ReferralMessage, JobTitle, JobURL,
                        0, MinSalary, SalaryCurrency, SalaryPeriod, PreferredLocations, @param4
                    FROM ReferralRequests WHERE RequestID = @param4`,
                    [childId, userId, referredAt, referrerOrgId, dto.requestID, childExtJobId]
                );
                ...
                // Convert hold to debit (first child that completes triggers payment)
                try {
                    const debitResult = await WalletService.convertHoldToDebit(dto.requestID);
                    console.log(`Open-to-any child ${childId} completed - parent hold converted: ₹${debitResult.Amount}`);
                } catch (holdErr: any) {
                ...
                try {
                    ...
                    if (seeker) {
                        await NotificationService.notifyReferralCompleted({
                            ...
                            seekerEmail: seeker.Email, referrerName: `${orgName} Employee`,
                            jobTitle: request.JobTitle || 'Job', companyName: orgName || 'Company'
                        });
                    }
                } catch {}
    ...
    /**
     * Convert a company-specific referral request to "Open to Any Company"
     * Charges the delta between current hold and open-to-any cost
     */
    static async convertToOpen(
        userId: string,
        applicantId: string,
        requestId: string,
        dto: { minSalary?: number; salaryCurrency?: string; salaryPeriod?: string; preferredLocations?: string }
    ): Promise<any> {
        try {
            // 1. Verify request belongs to applicant and is eligible
            const selectQuery = `
                SELECT rr.RequestID, rr.Status, rr.OpenToAnyCompany, rr.ExpiryTime,
                       h.Amount as CurrentHoldAmount, h.HoldID
                FROM ReferralRequests rr
            ...
            if (!['Pending', 'NotifiedToReferrers', 'Viewed', 'Claimed'].includes(request.Status)) {
                throw new ValidationError('Only pending/viewed/claimed requests can be converted');
            }

            if (request.OpenToAnyCompany) {
                throw new ValidationError('Request is already open to any company');
            }

            // 2. Calculate delta cost
            const openToAnyCost = await PricingService.getOpenToAnyCost();
            const currentHold = request.CurrentHoldAmount || 0;
            const deltaCost = Math.max(0, openToAnyCost - currentHold);

            // 3. If delta > 0, check balance and create additional hold or deduct
            if (deltaCost > 0) {
                const { availableBalance } = await WalletService.getAvailableBalance(userId);
                if (availableBalance < deltaCost) {
                    throw new ValidationError(`Insufficient balance. Need ₹${deltaCost} more (upgrade cost: ₹${openToAnyCost} - current hold: ₹${currentHold})`);
                }
            ...
            await dbService.executeQuery(
                `UPDATE ReferralRequests SET 
                    ...
                    JobID = NULL,
                    OrganizationID = NULL,
                    ExpiryTime = DATEADD(DAY, 14, GETUTCDATE()),
                    MinSalary = @param1,
                    SalaryCurrency = @param2,
                    SalaryPeriod = @param3,
                    PreferredLocations = @param4
                 WHERE RequestID = @param0`,
                [
                    requestId,
                    dto.minSalary || null,
                    dto.salaryCurrency || 'INR',
                    dto.salaryPeriod || 'Annual',
                    dto.preferredLocations || null,
                    openExtJobId
                ]
            );
            ...
            return {
                success: true,
                deltaCost,
                newTotalHold: openToAnyCost,
                message: deltaCost > 0
                    ? `Upgraded! ₹${deltaCost} additional hold placed (total: ₹${openToAnyCost})`
                    : `Upgraded to Open to Any Company at no extra cost`
            };

        } catch (error) {
            console.error('Error converting to open:', error);
            throw error;
        }
    }
    ...
    static async logStatusChange(
        ...
        try {
            ...
            if (shouldUpdateStatus) {
                ...
                // BUT: For open-to-any parents, do NOT update status/AssignedReferrerID — parent stays open
    ...
    static async getStatusHistory(requestId: string): Promise<{
        ...
        try {
            ...
            const requestData = {
                ...
                OpenToAnyCompany: !!row.OpenToAnyCompany,
            ...
            return {
                ...
                childReferrals: await this.getChildReferrals(requestId),
            };
        } catch (error) {
    ...
    /**
     * Get child referrals spawned from an open-to-any parent
     */
    static async getChildReferrals(parentRequestId: string): Promise<any[]> {
        try {
            const query = `
                SELECT rr.RequestID, rr.Status, rr.ReferredAt, rr.OrganizationID,
                       o.Name as CompanyName, o.LogoURL as CompanyLogo,
                       u.FirstName + ' ' + u.LastName as ReferrerName,
                       rp.FileURL as ProofFileURL, rp.Description as ProofDescription
                FROM ReferralRequests rr
