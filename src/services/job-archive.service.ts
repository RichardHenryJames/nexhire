...
export class JobArchiveService {
    ...
    static async archiveOldJobs(daysOld: number = 60, batchSize: number = 500): Promise<{
        ...
        let batchNumber = 0;
        const MAX_BATCHES = 10; // Cap at 10 batches per run to avoid Azure Function timeout

        try {
            console.log(`[Archive Service] Starting archival for jobs older than ${daysOld} days (batch size: ${batchSize}, max batches: ${MAX_BATCHES})...`);

            // LOOP: Process batches until no more old jobs remain or max batches reached
            let hasMoreJobs = true;
            while (hasMoreJobs && batchNumber < MAX_BATCHES) {
                batchNumber++;
