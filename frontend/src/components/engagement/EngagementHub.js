...
export default function EngagementHub({ navigation, dashboardStats = {}, applications = [], savedJobs = 0 }) {
  ...
  const computePulse = useCallback(() => {
    ...
    const baseActive = 40 + (hash(1) % 60); // 40–99 daily base
    ...
    return {
      ...
      resumesAnalyzed: Math.max(4, Math.round((200 + (hash(3) % 150)) * cumFraction)), // ~4 at midnight -> 200-349 by end of day
      resumesCreated: Math.max(3, Math.round((120 + (hash(4) % 80)) * cumFraction)),   // ~3 at midnight -> 120-199 by end of day
      linkedinOptimized: Math.max(2, Math.round((80 + (hash(5) % 60)) * cumFraction)), // ~2 at midnight -> 80-139 by end of day
      blindReviewed: Math.max(6, Math.round((300 + (hash(6) % 200)) * cumFraction)),   // ~6 at midnight -> 300-499 by end of day
    };
  }, []);
      ...
      <View style={[styles.pulseBar, { backgroundColor: colors.surface, borderColor: colors.border }]}>
        ...
        >
          <View style={styles.pulseChip}>
            <Text style={[styles.pulseValue, { color: colors.success }]}>{fakePulse.active}</Text>
            <Text style={[styles.pulseLabel, { color: colors.gray600 }]}> active now</Text>
          </View>
