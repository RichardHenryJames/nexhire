...
class RefOpenAPI {
   ...
   * Called when user re-accepts after a version change
   */
  async acceptTerms(termsVersion, privacyPolicyVersion) {
    return this.apiCall('/users/accept-terms', {
      method: 'POST',
...
if (!resumeId) {
 ...
 return normalized;
  ...
  // 💰 NEW: Get wallet holds (pending charges for referral requests)
  async getWalletHolds(status) {
    if (!this.token) return { success: false, error: 'Authentication required' };
    
    const params = new URLSearchParams();
    if (status) {
      params.append('status', status); // 'Active', 'Converted', or 'Released'
    }
    
    return this.apiCall(`/wallet/holds?${params}`);
  }

  // 💰 NEW: Create wallet recharge order
  async createWalletRechargeOrder(amount, currencyId = 4, packId = null, promoCode = null) {
    if (!this.token) return { success: false, error: 'Authentication required' };
    
    return this.apiCall('/wallet/recharge/create-order', {
      method: 'POST',
      body: JSON.stringify({ amount, currencyId, packId, promoCode }),
    });
  }
    ...
    try {
      ...
      if (typeof requestData === 'object') {
        ...
        payload = {
          ...
          openToAnyCompany: requestData.openToAnyCompany || undefined,
        ...
        // ✅ FIXED: Only require jobTitle and organizationId for non-openToAny external referrals
        if (hasExtJobID && !isOpenToAny && (!payload.jobTitle || !payload.organizationId)) {
  ...
  async convertReferralToOpen(requestId, data) {
    if (!this.token) {
      return { success: false, error: 'Authentication required' };
    }
    if (!requestId) {
    ...
    try {
      const result = await this.apiCall(`/referral/requests/${requestId}/convert-to-open`, {
        method: 'POST',
        body: JSON.stringify(data),
      });
      return result;
    } catch (error) {
      console.error('❌ Convert to open failed:', error.message);
      throw error;
    }
  }
  ...
  /**
   * Validate a promo code for wallet recharge
  ...
  async validatePromoCode(code, amount) {
    ...
    return this.apiCall('/wallet/validate-promo', {
      method: 'POST',
   ...
   * 
   * @param {Object} fileData - Resume file info { name, size, file (web) or uri (native) }
   * @param {Object} jobData - Job info { jobId?, jobUrl?, jobDescription?, userId? }
   * @returns {Promise<Object>} Analysis result with matchScore, tips, missingKeywords, strengths
   */
  async analyzeResume(fileData, jobData) {
    try {
      const formData = new FormData();
      
      // Check if using resume from profile (has URL) or uploaded file
      if (fileData.isFromProfile && fileData.resumeUrl) {
        // Resume from user's profile - send URL instead of file
        formData.append('resumeUrl', fileData.resumeUrl);
        formData.append('resumeFileName', fileData.name);
        if (fileData.resumeId) {
          formData.append('resumeId', fileData.resumeId);
        }
      ...
      if (jobData.jobUrl) {
        formData.append('jobUrl', jobData.jobUrl);
      }
      ...
      const response = await fetch(`${this.baseURL}/tools/resume-analyzer`, {
        method: 'POST',
        headers,
        body: formData,
      ...
      if (!response.ok) {
        return {
          ...
          // Pass through payment-related fields from backend (402 responses)
          requiresPayment: data.requiresPayment || false,
          cost: data.cost || 0,
          freeUsesRemaining: data.freeUsesRemaining,
        };
      }
