...
const getTypeColors = (colors) => ({
  ...
  job_recommendations: colors.primary,
  referrer_digest: colors.accent,
  become_verified: colors.success,
  referral_expiring: colors.warning,
});
...
export default function NotificationsScreen() {
  ...
  const handleNotificationPress = async (notification) => {
    ...
    } else if (url.startsWith('/referrals/tracking/')) {
      const reqId = url.split('/referrals/tracking/')[1];
      navigation.navigate('ReferralTracking', { requestId: reqId });
    ...
    } else if (url.startsWith('/provide-referral')) {
      navigation.navigate('ProvideReferral');
    } else if (url.startsWith('/get-verified')) {
      navigation.navigate('GetVerified');
    } else if (url.startsWith('/resume-builder')) {
