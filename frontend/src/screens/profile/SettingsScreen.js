...
export default function SettingsScreen({ navigation, route }) {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        if (userType === 'JobSeeker') {
          ...
          let preferredRoles = [];
          try { preferredRoles = data.PreferredRoles ? data.PreferredRoles.split(',').map(s => s.trim()).filter(Boolean) : []; } catch {}
  ...
  const loadPreferencesReferenceData = async () => {
    try {
      const response = await refopenAPI.getBulkReferenceMetadata(['Industry', 'WorkplaceType', 'JobType', 'JobRole']);
      if (response?.success && response?.data) {
        setReferenceData({
          industries: response.data.Industry || [],
          workTypes: response.data.WorkplaceType || [],
          jobTypes: response.data.JobType || [],
          jobRoles: response.data.JobRole || [],
  ...
  const savePreferences = async () => {
    try {
      await updateProfileSmart({
        preferredRoles: jobSeekerProfile.preferredRoles.join(','),
    ...
    >
      <View style={styles.modalContainer}>
        <View style={styles.modalInner}>
          ...
          <View style={styles.inputGroup}>
            <Text style={styles.inputLabel}>Preferred Roles</Text>
            <TextInput
            ...
            <View style={styles.chipContainer}>
              ...
              {(referenceData.jobRoles || [])
                .filter(role => {
                  ...
                  if (jobSeekerProfile.preferredRoles.includes(val)) return true;
                  if (!rolesSearch) return false;
                  return val.toLowerCase().includes(rolesSearch.toLowerCase());
                })
                .map((role) => (
                <TouchableOpacity
                  key={role.MetadataID || role.Value}
                  style={[
                    styles.chip,
                    jobSeekerProfile.preferredRoles.includes(role.Value) && styles.chipSelected
                  ]}
                  onPress={() => {
                    const val = role.Value;
                    setJobSeekerProfile(prev => ({
                      ...prev,
                      preferredRoles: prev.preferredRoles.includes(val)
                        ? prev.preferredRoles.filter(v => v !== val)
                        : [...prev.preferredRoles, val]
                    }));
                  }}
                >
                  <Text style={[
                    styles.chipText,
                    jobSeekerProfile.preferredRoles.includes(role.Value) && styles.chipTextSelected
                  ]}>
                    {role.Value}
                  </Text>
                </TouchableOpacity>
  ...
  return (
    ...
    <View style={styles.container}>
      <View style={styles.innerContainer}>
        ...
        {isJobSeeker && (
          <TouchableOpacity 
            style={{ flexDirection: 'row', alignItems: 'center', padding: 16, marginHorizontal: 16, marginBottom: 12, borderRadius: 12, borderWidth: 1, borderColor: subscription?.isPro ? '#D4A45A' + '40' : colors.border, backgroundColor: subscription?.isPro ? '#D4A45A' + '08' : colors.surface }}
            onPress={() => navigation.navigate('Pricing')}
          >
            <View style={{ width: 44, height: 44, borderRadius: 12, backgroundColor: subscription?.isPro ? '#D4A45A' + '20' : colors.primary + '15', justifyContent: 'center', alignItems: 'center' }}>
              <Ionicons name={subscription?.isPro ? 'diamond' : 'diamond-outline'} size={22} color={subscription?.isPro ? '#D4A45A' : colors.primary} />
            </View>
            <View style={{ flex: 1, marginLeft: 14 }}>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                <Text style={{ fontSize: 15, fontWeight: '700', color: colors.text }}>{subscription?.isPro ? 'RefOpen Pro' : 'Free Plan'}</Text>
                {subscription?.isPro && (
                  <View style={{ backgroundColor: '#D4A45A', paddingHorizontal: 8, paddingVertical: 2, borderRadius: 6 }}>
                    <Text style={{ fontSize: 10, fontWeight: '700', color: '#1a1a1a' }}>PRO</Text>
                  </View>
                )}
              </View>
