    ...
    return (
      ...
      >
        ...
        <View style={styles.holdContent}>
          ...
          {/* Expiry countdown for active holds */}
          {item.Status === 'Active' && (() => {
            const expiryDate = item.ExpiryTime
              ? new Date(item.ExpiryTime)
              : new Date(new Date(item.CreatedAt).getTime() + 14 * 24 * 60 * 60 * 1000);
            const diffMs = expiryDate - new Date();
            if (diffMs <= 0) return <Text style={[styles.holdResolvedDate, { color: colors.error }]}>Expiring soon</Text>;
            ...
            return (
              <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 2 }}>
                <Ionicons name="timer-outline" size={12} color={expiryColor} style={{ marginRight: 3 }} />
                <Text style={[styles.holdResolvedDate, { color: expiryColor }]}>{expiryText}</Text>
              </View>
            );
          })()}
        ...
        <View style={styles.summaryCard}>
          ...
          {summary.activeHoldsCount > 0 && (
            <Text style={styles.summaryHint}>
              {summary.activeHoldsCount} active hold{summary.activeHoldsCount > 1 ? 's' : ''} • Auto-released on expiry
            </Text>
          )}
        </View>
      )}
  ...
  const renderEmpty = () => (
    <View style={styles.emptyContainer}>
      ...
      <Text style={styles.emptySubtext}>
        {filter === 'Active'
          ? 'Holds are created when you request referrals.\nThey\'re auto-released if no referral before expiry.'
          : filter === 'Converted'
