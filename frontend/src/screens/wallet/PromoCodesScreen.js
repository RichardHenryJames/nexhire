...
const PromoCard = ({ promo, colors, onApply }) => {
  ...
  const ineligible = !promo.eligible && !exhausted;
  ...
  return (
    <View style={{
      backgroundColor: (exhausted || ineligible) ? colors.surface + '80' : colors.surface,
      borderRadius: 14,
      padding: 14,
      marginBottom: 10,
      borderWidth: 1,
      borderColor: promo.recommended ? colors.primary + '40' : colors.border,
      opacity: (exhausted || ineligible) ? 0.55 : 1,
    }}>
      {/* Top row: Code badge + bonus */}
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
          <View style={{
            backgroundColor: (exhausted || ineligible) ? colors.gray300 : colors.primary + '15',
            paddingHorizontal: 10,
            paddingVertical: 5,
            borderRadius: 6,
            borderWidth: 1,
            borderColor: (exhausted || ineligible) ? colors.gray400 : colors.primary + '30',
            borderStyle: 'dashed',
          }}>
            <Text style={{
              fontSize: 14,
              fontWeight: '800',
              color: (exhausted || ineligible) ? colors.gray500 : colors.primary,
              letterSpacing: 1.5,
            }}>{promo.code}</Text>
          </View>
        ...
        <View style={{
          backgroundColor: (exhausted || ineligible) ? colors.gray200 : colors.successBg,
          paddingHorizontal: 8,
          paddingVertical: 3,
          borderRadius: 10,
        }}>
          <Text style={{
            fontSize: 12,
            fontWeight: '700',
            color: (exhausted || ineligible) ? colors.gray500 : colors.success,
          }}>{bonusLabel}</Text>
        </View>
      </View>
      ...
      {exhausted ? (
        ...
        <View style={{
          ...
          <Text style={{ fontSize: 12, fontWeight: '600', color: colors.gray500 }}>
            {promo.ineligibleReason || 'Not eligible for this code'}
          </Text>
        </View>
      ) : (
        <TouchableOpacity
...
const PromoCodesScreen = ({ navigation }) => {
  ...
  const others = promoCodes.filter(p => !p.recommended && p.eligible && !p.exhausted);
  const ineligible = promoCodes.filter(p => !p.eligible && !p.exhausted);
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.content}>
        ...
        >
          ...
          {/* Ineligible codes — greyed out */}
          {ineligible.length > 0 && (
            <View style={{ marginBottom: 16 }}>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: 10 }}>
                <Ionicons name="lock-closed-outline" size={16} color={colors.gray400} />
                <Text style={{ fontSize: 15, fontWeight: '700', color: colors.gray400 }}>Not Eligible</Text>
              </View>
