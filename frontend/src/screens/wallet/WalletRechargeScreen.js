    ...
    <View style={styles.container}>
      ...
      <Modal visible={showFaqModal} transparent animationType="fade" onRequestClose={() => setShowFaqModal(false)}>
        <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.7)', justifyContent: 'center', alignItems: 'center', padding: 24 }}>
          <View style={{ backgroundColor: colors.surface, borderRadius: 16, borderWidth: 1, borderColor: colors.border, padding: 20, width: '100%', maxWidth: 560, maxHeight: '80%' }}>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <Text style={{ fontSize: 16, fontWeight: '700', color: colors.text }}>Frequently Asked Questions</Text>
            ...
            <ScrollView showsVerticalScrollIndicator={false} style={{ maxHeight: 500 }}>
              {[
                ...
                },
                {
                  ...
                  a: `• Referral request — from ₹${pricing.referralRequestCost} (varies by company)\n• Open-to-any-company referral — ₹${pricing.openToAnyReferralCost}\n• AI Job Recommendations (${pricing.aiAccessDurationDays} days) — ₹${pricing.aiJobsCost}\n• Profile Views (${pricing.profileViewAccessDurationDays} days) — ₹${pricing.profileViewCost}`,
                },
                {
                  q: 'What happens when I request a referral?',
                  a: 'When you request a referral, the amount is placed on hold (not deducted). If a referrer picks up your request, the hold converts to a debit. If no one picks it up within 14 days, the full amount is automatically released back to your wallet.',
                },
                {
                ...
                >
                  <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                    <Text style={{ flex: 1, fontSize: 14, fontWeight: '600', color: colors.text, marginRight: 8 }}>{item.q}</Text>
                    <Ionicons name={expandedFaq === idx ? 'chevron-up' : 'chevron-down'} size={18} color={colors.textSecondary} />
                  </View>
                  {expandedFaq === idx && (
                    <Text style={{ fontSize: 13, color: colors.textSecondary, lineHeight: 20, marginTop: 8 }}>{item.a}</Text>
                  )}
                </TouchableOpacity>
              ))}
            </ScrollView>
