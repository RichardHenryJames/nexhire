...
| **Post Jobs** | Employers create and manage job listings | Free |
| **Save Jobs** | Bookmark jobs for later | Free |
| **Request Referral** | Get internal referral from company employee | ₹49 per request |
| **External Referral** | Request referral for jobs not in our system | ₹49 per request |
...
|---------|-------|----------|
| Referral Request | ₹49 | Per request |
| AI Job Recommendations | ₹99 | 15 days access |
| Profile Views Access | ₹29 | 7 days access |
| Job Publish (Employer) | Free | Per job |
| Welcome Bonus | ₹0 | On signup |
| Referral Signup Bonus | +₹25 | Both parties |
...
| POST | `/referral/requests` | Create referral request | ✅ (₹49) |
...
| Referral Request | -₹49 | Create request |
| AI Recommendations | -₹99 | Use AI feature (15 days) |
| Referral Reward | +Variable | Complete referral |
| Welcome Bonus | ₹0 | New registration |
| Referral Signup | +₹25 | Referred user joins |
...
```
Jobs → JobDetails → Ask Referral → ConfirmCost (₹49) → ResumeSelect → Submit → MyReferralRequests
```

### Referral Request Flow (External Job)
```
AskReferral → Enter Company/Job URL → ConfirmCost (₹49) → ResumeUpload → Submit → MyReferralRequests
```
