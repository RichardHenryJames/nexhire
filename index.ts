...
// NEW: Convert referral request to "Open to Any Company"
app.http("referral-convert-to-open", {
  methods: ["POST", "OPTIONS"],
  authLevel: "anonymous",
  route: "referral/requests/{requestId}/convert-to-open",
  handler: convertReferralToOpen,
});
...
import { submitBlindReview, getBlindReviewStatus, getBlindReviewHistory, getBlindReviewPending, submitBlindReviewResponse, getBlindReviewMyReviews, cancelBlindReview, aiPrefillBlindReview } from "./src/controllers/blind-review.controller";
...
app.http("tools-blind-review-ai-prefill", {
  methods: ["POST", "OPTIONS"],
  authLevel: "anonymous",
  route: "tools/blind-review/ai-prefill/{id}",
  handler: aiPrefillBlindReview,
});
  ...
  handler: withErrorHandling(async (req, context) => {
    try {
      ...
      try {
        ...
        // 🤖 AI ENRICHMENT: Run after scraping (same as timer trigger)
        let enrichmentSummary: any = null;
        try {
          const { JobDescriptionEnricherService } = await import(
            "./src/services/jobDescriptionEnricher.service"
          );
          console.log("🤖 Starting AI job description enrichment...");
          ...
          enrichmentSummary = {
            ...
            failed: enrichResult.totalFailed,
            timeSeconds: Math.round(enrichResult.executionTimeMs / 1000),
            errors: enrichResult.errors.slice(0, 5),
          };
          console.log(`🤖 AI enrichment: ${enrichResult.totalEnriched}/${enrichResult.totalProcessed} enriched`);
        } catch (enrichError: any) {
          console.warn("⚠️  AI enrichment error (non-fatal):", enrichError.message);
...
// Standalone AI enrichment endpoint (Admin only)
// Can be triggered independently without scraping — useful for enriching
// existing AIEnriched=0 jobs or testing the enrichment service
app.http("job-enrich-trigger", {
  methods: ["POST", "OPTIONS"],
  authLevel: "anonymous",
  route: "jobs/enrich/trigger",
  handler: withErrorHandling(async (req, context) => {
    try {
      const authHeader = req.headers.get("authorization");
      if (!authHeader?.startsWith("Bearer ")) {
      ...
      const { AuthService } = await import("./src/services/auth.service");
      const payload = await AuthService.verifyToken(authHeader.replace("Bearer ", ""));
      if (payload.userType !== "Admin") {
        return { status: 403, jsonBody: { success: false, error: "Admin access required" } };
      }

      console.log("Manual AI enrichment triggered by admin:", payload.userId);
      ...
      return {
        ...
        jsonBody: {
          success: true,
          message: `AI enrichment complete: ${enrichResult.totalEnriched}/${enrichResult.totalProcessed} enriched`,
          data: {
            enriched: enrichResult.totalEnriched,
            processed: enrichResult.totalProcessed,
            failed: enrichResult.totalFailed,
            timeSeconds: Math.round(enrichResult.executionTimeMs / 1000),
            errors: enrichResult.errors.slice(0, 10),
          },
        },
      };
    } catch (error: any) {
      console.error("AI enrichment trigger failed:", error);
...
app.timer("jobEnrichmentTimer", {
  ...
  handler: async (myTimer: Timer, context: InvocationContext) => {
    ...
    if (appEnv !== 'production' && appEnv !== 'prod') {
      context.log(`[AI Enrichment] Skipping — not production (env: ${appEnv})`);
      return;
    }

    const startTime = Date.now();
    context.log("========================================================================");
    context.log("        AI JOB DESCRIPTION ENRICHMENT TIMER");
    context.log("========================================================================");
    ...
    try {
      ...
      context.log(`   Processed: ${enrichResult.totalProcessed}`);
      context.log(`   Enriched:  ${enrichResult.totalEnriched}`);
      context.log(`   Failed:    ${enrichResult.totalFailed} (will retry next run)`);
      context.log(`   Duration:  ${duration}s`);

      if (enrichResult.errors.length > 0) {
        context.warn(`   Errors (first 5):`);
        enrichResult.errors.slice(0, 5).forEach((err, i) => context.warn(`     ${i + 1}. ${err}`));
      }
    } catch (error: any) {
      context.error(`AI Enrichment failed: ${error.message}`);
    }
  },
});

// ========================================================================
// TIMER TRIGGER - JOB ARCHIVAL (Every 6 hours)
// ========================================================================
...
// ========================================================================
// TIMER TRIGGER - REFERRER NOTIFICATION EMAILS (11:30 AM IST = 6:00 AM UTC, Daily)
// ========================================================================
...
app.timer("jobScraperTimer", {
  ...
  handler: async (myTimer: Timer, context: InvocationContext) => {
    ...
    try {
      ...
      // NOTE: AI enrichment now runs on its own separate timer (every 2 hours)
      // See jobEnrichmentTimer below
...
app.timer("expiringRequestNudgeTimer", {
  schedule: "0 30 4 * * *", // 4:30 AM UTC = 10:00 AM IST daily
  handler: async (myTimer: Timer, context: InvocationContext) => {
    const startTime = Date.now();
    const executionId = `expiry_nudge_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;

    context.log("========================================================================");
    context.log("         EXPIRING REQUEST NUDGE EMAIL TIMER");
    context.log("========================================================================");
    context.log(`Execution ID: ${executionId}`);
    context.log(`Triggered at: ${new Date().toISOString()}`);
    context.log(`Schedule: 10:00 AM IST (4:30 AM UTC) - Daily`);
    context.log(`Past Due: ${myTimer.isPastDue ? "Yes (catching up)" : "No"}`);
    ...
    try {
      const { ExpiringRequestNudgeService } = await import("./src/services/expiringRequestNudgeService");

      context.log("\n🚀 Starting expiring request nudge email process...\n");

      const result = await ExpiringRequestNudgeService.processExpiringRequests();

      const runEndTime = new Date();
      const duration = Date.now() - startTime;

      // Log the run
      await ExpiringRequestNudgeService.logEmailRun(
        executionId,
        runStartTime,
        runEndTime,
        result,
      ...
      context.log("\n========================================================================");
      context.log("        EXPIRING REQUEST NUDGE TIMER COMPLETED");
      context.log("========================================================================");
      context.log(`Total Eligible: ${result.totalEligible}`);
      context.log(`Emails Sent: ${result.emailsSent}`);
      context.log(`Emails Failed: ${result.emailsFailed}`);
      ...
      const runEndTime = new Date();
      context.error("❌ EXPIRING REQUEST NUDGE TIMER FAILED");
      context.error(`   Error: ${error.message}`);
