...
Write-Host "Seeding enhanced sample jobs into NexHire DB (more varied data for personalization testing)..." -ForegroundColor Green
...
-- Organizations catalog (expanded for variety: added healthcare, education, finance, etc.)
DECLARE @Orgs TABLE(
...
,('MediTech Health','Private','Healthcare','501-1000','https://meditech.example','Boston, US')  -- New: Healthcare for field variety
,('EduCorp','Public','Education','1001-5000','https://educorp.example','Sydney, AU')  -- New: Education
,('FinSecure','Startup','Finance','51-200','https://finsecure.example','Mumbai, IN')  -- New: Finance
,('EcoManufacture','Private','Manufacturing','201-500','https://ecomanufacture.example','Berlin, DE')  -- New: Manufacturing
,('EntertainHub','Private','Entertainment','501-1000','https://entertainhub.example','Los Angeles, US');  -- New: Entertainment
...
-- Poster users per org (expanded to match new orgs)
...
,('recruiter+meditech@nexhire.test','MediTech','Recruiter')  -- New
,('recruiter+educorp@nexhire.test','EduCorp','Recruiter')  -- New
,('recruiter+finsecure@nexhire.test','FinSecure','Recruiter')  -- New
,('recruiter+ecomanufacture@nexhire.test','EcoManufacture','Recruiter')  -- New
,('recruiter+entertainhub@nexhire.test','EntertainHub','Recruiter');  -- New
...
DECLARE @EUR INT = (SELECT TOP 1 CurrencyID FROM Currencies WHERE Code='EUR');  -- New for variety
DECLARE @AUD INT = (SELECT TOP 1 CurrencyID FROM Currencies WHERE Code='AUD');  -- New for variety
...
-- Jobs catalog (enhanced: 50+ rows for more data/variety; includes internships, fresher, senior/exec; diverse fields/locations)
-- This variety supports testing personalization: e.g., match user exp (0 for students/interns, 6+ for seniors), skills, departments (avoid showing medical to SWE users).
DECLARE @Jobs TABLE(
...
('Acme Corp','recruiter+acme@nexhire.test','Junior Software Engineer','Full-time','Hybrid','Engineering','Entry-level development','Code features, bug fixes','Mentorship, training','Bengaluru, KA','India','Bengaluru',800000,1200000,'INR','Annual',0,2,'javascript,python,basics'),  -- New: Fresher level
...
('Globex Corporation','recruiter+globex@nexhire.test','Finance Analyst','Full-time','Remote','Finance','Financial modeling','Reports, forecasts','Remote flexibility','Pune, MH','India','Pune',1500000,2500000,'INR','Annual',2,5,'excel,finance,analytics'),  -- New: Non-tech field
...
('Innotech Solutions','recruiter+innotech@nexhire.test','Intern - Data Analyst','Internship','Remote','Data','Learn data tools','Basic analysis, reports','Stipend, certificate','Hyderabad, TS','India','Hyderabad',15000,25000,'INR','Monthly',0,1,'excel,sql,intern'),  -- New: Internship
...
('Umbrella Labs','recruiter+umbrella@nexhire.test','Biotech Research Intern','Internship','Onsite','R&D','Lab assistance','Experiments, data entry','Mentorship','San Francisco, CA','United States','San Francisco',2000,3000,'USD','Monthly',0,1,'biology,lab,intern'),  -- New: Internship in different field
...
('Stark Industries','recruiter+stark@nexhire.test','Executive Director - R&D','Full-time','Hybrid','Leadership','Lead innovation','Strategy, team mgmt','Equity, bonuses','New York, NY','United States','New York',250000,350000,'USD','Annual',10,20,'leadership,ai,exec'),  -- New: Executive level
...
('Wayne Enterprises','recruiter+wayne@nexhire.test','Part-time IT Support','Part-time','Onsite','IT','Basic support','Helpdesk, troubleshooting','Flexible hours','London','United Kingdom','London',20000,30000,'GBP','Annual',1,3,'support,parttime'),  -- New: Part-time
...
('Cyberdyne Systems','recruiter+cyberdyne@nexhire.test','Senior Robotics Lead','Full-time','Hybrid','Engineering','Lead robotics teams','Design, oversee projects','Leadership perks','Tokyo','Japan','Tokyo',15000000,22000000,'JPY','Annual',8,15,'robotics,leadership,senior'),  -- New: Senior level
...
('NexusSoft','recruiter+nexussoft@nexhire.test','Contract UI Designer','Contract','Remote','Design','Short-term UI work','Prototypes, iterations','Project-based pay','Singapore','Singapore','Singapore',5000,8000,'SGD','Monthly',2,5,'ui,figma,contract'),  -- New: Contract type
...
('MediTech Health','recruiter+meditech@nexhire.test','Nurse Practitioner','Full-time','Onsite','Healthcare','Patient care','Assessments, treatments','Health benefits','Boston, MA','United States','Boston',80000,110000,'USD','Annual',2,6,'nursing,healthcare,patient'),  -- New: Medical field
...
('Acme Corp','recruiter+acme@nexhire.test','Executive Software Architect','Full-time','Remote','Engineering','Architecture design','Scalability, tech strategy','Equity','Bengaluru, KA','India','Bengaluru',5000000,7000000,'INR','Annual',10,15,'architecture,cloud,exec'),  -- New: Executive
...
-- Attach a few skills to jobs if available (expanded for variety; supports skill-based matching in personalization)
...
DECLARE @SkillSql INT = (SELECT TOP 1 SkillID FROM Skills WHERE Name='SQL');  -- New
DECLARE @SkillNursing INT = (SELECT TOP 1 SkillID FROM Skills WHERE Name='Nursing');  -- New: For field-specific
...
BEGIN
  ...
  FROM Jobs j
  WHERE j.Title LIKE '%Nurse%' OR j.Title LIKE '%Healthcare%'
...
try {
  ...
  Write-Host "You can now refresh the Jobs screen and test personalization logic." -ForegroundColor Cyan
}
catch {
