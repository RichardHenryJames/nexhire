...
class JobApplicationService {
    ...
    static async applyForJob(applicationData, applicantUserId) {
        const validatedData = (0, validation_1.validateRequest)(validation_1.jobApplicationSchema, applicationData);
        ...
        let applicantResult = await database_service_1.dbService.executeQuery(applicantQuery, [applicantUserId]);
        let applicantId;
        if (!applicantResult.recordset || applicantResult.recordset.length === 0) {
            ...
            applicantId = auth_service_1.AuthService.generateUniqueId();
            ...
            `;
            await database_service_1.dbService.executeQuery(createApplicantQuery, [applicantId, applicantUserId]);
        }
        else {
        ...
        `;
        const jobResult = await database_service_1.dbService.executeQuery(jobQuery, [validatedData.jobID]);
        if (!jobResult.recordset || jobResult.recordset.length === 0) {
            throw new validation_1.NotFoundError('Job not found or not available for applications');
        }
        const job = jobResult.recordset[0];
        // Check application deadline (only if set)
        if (job.ApplicationDeadline && new Date(job.ApplicationDeadline) < new Date()) {
            throw new validation_1.ValidationError('Application deadline has passed');
        }
        // Check max applications limit (only if set)
        if (job.MaxApplications && job.CurrentApplications >= job.MaxApplications) {
            throw new validation_1.ValidationError('Maximum number of applications reached for this job');
        }
        ...
        `;
        const existingResult = await database_service_1.dbService.executeQuery(existingApplicationQuery, [validatedData.jobID, applicantId]);
        if (existingResult.recordset && existingResult.recordset.length > 0) {
            throw new validation_1.ConflictError('You have already applied for this job');
        }
        // Generate application ID
        const applicationId = auth_service_1.AuthService.generateUniqueId();
        ...
        try {
            const result = await database_service_1.dbService.executeQuery(insertQuery, parameters);
    ...
    static async getApplicationsByUser(userId, params) {
        ...
        try {
            ...
            if (!applicantResult.recordset || applicantResult.recordset.length === 0) {
                ...
                `;
                try {
                    await database_service_1.dbService.executeQuery(createApplicantQuery, [applicantId, userId]);
            ...
            const countResult = await database_service_1.dbService.executeQuery(countQuery, [applicantId]);
            ...
            `;
            const dataResult = await database_service_1.dbService.executeQuery(dataQuery, [applicantId, offset.toString(), pageSize.toString()]);
            return {
    ...
    static async getApplicationsByJob(jobId, employerUserId, params) {
        ...
        try {
            ...
            `;
            const accessResult = await database_service_1.dbService.executeQuery(accessQuery, [jobId, employerUserId]);
            ...
            const countResult = await database_service_1.dbService.executeQuery(countQuery, queryParams);
            const total = countResult.recordset[0]?.total || 0;
            const totalPages = Math.ceil(total / pageSize);
            if (total === 0) {
                return { applications: [], total: 0, totalPages: 0 };
            ...
            const dataResult = await database_service_1.dbService.executeQuery(dataQuery, queryParams);
            return {
                applications: dataResult.recordset || [],
                total,
                totalPages
    ...
    static async updateApplicationStatus(applicationId, statusId, employerUserId, notes) {
        ...
        `;
        const accessResult = await database_service_1.dbService.executeQuery(accessQuery, [applicationId, employerUserId]);
        if (!accessResult.recordset || accessResult.recordset.length === 0) {
            throw new validation_1.ValidationError('Access denied to this application');
        }
        // Update application status
        const updateQuery = `
            ...
            WHERE ja.ApplicationID = @param0;
        `;
        const result = await database_service_1.dbService.executeQuery(updateQuery, [applicationId, statusId]);
        if (!result.recordset || result.recordset.length === 0) {
    ...
    static async withdrawApplication(applicationId, userId) {
        ...
        `;
        const accessResult = await database_service_1.dbService.executeQuery(accessQuery, [applicationId, userId]);
        if (!accessResult.recordset || accessResult.recordset.length === 0) {
            throw new validation_1.ValidationError('Access denied to this application');
        }
        ...
        const updateQuery = `
            ...
            WHERE ApplicationID = @param0
        `;
        await database_service_1.dbService.executeQuery(updateQuery, [applicationId]);
    }
    // Get application details
    static async getApplicationDetails(applicationId, userId) {
        const query = `
            SELECT 
                ja.*,
                j.Title as JobTitle,
            ...
            WHERE ja.ApplicationID = @param0 AND a.UserID = @param1
        `;
        const result = await database_service_1.dbService.executeQuery(query, [applicationId, userId]);
        return result.recordset && result.recordset.length > 0 ? result.recordset[0] : null;
    }
    // Private helper methods
    static async createApplicationTracking(applicationId, statusTypeId) {
        const trackingId = auth_service_1.AuthService.generateUniqueId();
        const query = `
        ...
        `;
        await database_service_1.dbService.executeQuery(query, [trackingId, applicationId, statusTypeId]);
    }
    static async updateApplicationTracking(applicationId, updatedBy, updateData) {
        ...
        const query = `
            ...
            WHERE ApplicationID = @param0
        `;
        await database_service_1.dbService.executeQuery(query, [
            applicationId,
            notes,
            screeningScore,
            interviewStage,
    ...
    static async updateApplicantLastApplication(applicantId) {
        const query = `
            UPDATE Applicants 
            SET LastJobAppliedAt = GETUTCDATE()
            WHERE ApplicantID = @param0
        `;
        await database_service_1.dbService.executeQuery(query, [applicantId]);
    }
    // Get application statistics for dashboard
    static async getApplicationStats(userId, userType) {
        if (userType === config_1.appConstants.userTypes.JOB_SEEKER) {
            return await this.getJobSeekerApplicationStats(userId);
        }
        else if (userType === config_1.appConstants.userTypes.EMPLOYER) {
            return await this.getEmployerApplicationStats(userId);
        }
        return {};
    }
    static async getJobSeekerApplicationStats(userId) {
        const query = `
            SELECT 
    ...
    static async getEmployerApplicationStats(userId) {
        const query = `
            SELECT 
