...
class JobService {
    // Create new job
    static async createJob(jobData, postedByUserID, organizationID) {
        const validatedData = (0, validation_1.validateRequest)(validation_1.jobCreateSchema, jobData);
        // Generate job ID
        const jobId = auth_service_1.AuthService.generateUniqueId();
        ...
        `;
        const result = await database_service_1.dbService.executeQuery(query, values);
    ...
    static async getJobById(jobId) {
        const query = `
            ...
            WHERE j.JobID = @param0
        `;
        const result = await database_service_1.dbService.executeQuery(query, [jobId]);
    ...
    static async updateJob(jobId, updateData, userId) {
        ...
        if (!existingJob) {
            throw new validation_1.NotFoundError('Job not found');
        }
        ...
        if (existingJob.PostedByUserID !== userId) {
            ...
            `;
            const permissionResult = await database_service_1.dbService.executeQuery(permissionQuery, [userId, existingJob.OrganizationID]);
            if (!permissionResult.recordset || permissionResult.recordset.length === 0) {
                throw new validation_1.ValidationError('Insufficient permissions to update this job');
            }
        }
        const allowedFields = [
        ...
        const updateFields = Object.keys(updateData)
            ...
            .join(', ');
        if (!updateFields) {
            throw new validation_1.ValidationError('No valid fields to update');
        }
        ...
        await database_service_1.dbService.executeQuery(query, parameters);
        // Return updated job
    ...
    static async publishJob(jobId, userId) {
        ...
        if (job.PostedByUserID !== userId) {
            const permissionQuery = `
                ...
                WHERE e.UserID = @param0 AND e.OrganizationID = @param1 AND e.CanPostJobs = 1
            `;
            const permissionResult = await database_service_1.dbService.executeQuery(permissionQuery, [userId, job.OrganizationID]);
            if (!permissionResult.recordset || permissionResult.recordset.length === 0) {
                throw new validation_1.ValidationError('Insufficient permissions to publish this job');
            }
        }
        ...
        const query = `
            ...
            WHERE JobID = @param0
        `;
        await database_service_1.dbService.executeQuery(query, [jobId]);
    ...
    static async closeJob(jobId, userId) {
        ...
        if (job.PostedByUserID !== userId) {
            ...
            const permissionResult = await database_service_1.dbService.executeQuery(permissionQuery, [userId, job.OrganizationID]);
            if (!permissionResult.recordset || permissionResult.recordset.length === 0) {
                throw new validation_1.ValidationError('Insufficient permissions to close this job');
            }
        }
        const query = `
            UPDATE Jobs 
    ...
    static async deleteJob(jobId, userId) {
        ...
        if (job.PostedByUserID !== userId) {
            throw new validation_1.ValidationError('Only job owner can delete the job');
        }
        // Check if job has applications
        const applicationQuery = 'SELECT COUNT(*) as count FROM JobApplications WHERE JobID = @param0';
        const applicationResult = await database_service_1.dbService.executeQuery(applicationQuery, [jobId]);
        if (applicationResult.recordset[0].count > 0) {
            throw new validation_1.ValidationError('Cannot delete job with existing applications');
        }
        const query = 'DELETE FROM Jobs WHERE JobID = @param0';
        await database_service_1.dbService.executeQuery(query, [jobId]);
    }
    // Get jobs by organization
    static async getJobsByOrganization(organizationId, params) {
        ...
        const countResult = await database_service_1.dbService.executeQuery(countQuery, [organizationId]);
        const total = countResult.recordset[0].total;
        const totalPages = Math.ceil(total / pageSize);
        // Get paginated results
        const offset = (page - 1) * pageSize;
        ...
        `;
        const dataResult = await database_service_1.dbService.executeQuery(dataQuery, [organizationId, offset, pageSize]);
        return {
            jobs: dataResult.recordset || [],
            total,
            totalPages
    ...
    static async searchJobs(searchParams) {
        try {
            const { search, // 'search' parameter from controller
            q, // 'q' parameter from URL query string
            location, jobType, experienceLevel, isRemote, salaryMin, salaryMax, page = 1, pageSize = 20 } = searchParams;
