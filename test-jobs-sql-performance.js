...
async function runTests(pool, userId = null) {
    ...
    // NEW TESTS - Frontend Filters
    
    // Test 7: Workplace Filter (Remote/Hybrid/On-site)
    console.log(`\n${colors.bright}${colors.cyan}???????????????????????????????????????????${colors.reset}`);
    console.log(`${colors.bright}${colors.cyan}   TEST 7: Workplace Filter (Remote)${colors.reset}`);
    console.log(`${colors.bright}${colors.cyan}???????????????????????????????????????????${colors.reset}\n`);
    
    const { whereClause: where7, params: params7 } = buildJobFilters({ workplaceTypeIds: '3' }, userId);
    const workplaceQuery7 = `
        SELECT COUNT(*) as total
        FROM Jobs j
        INNER JOIN Organizations o ON j.OrganizationID = o.OrganizationID
        ${where7}
    `;
    results.push(await executeQueryWithTiming(pool, 'Workplace: Remote (WorkplaceTypeID=3)', workplaceQuery7, params7));
    
    // Test 8: JobType Filter (Full-Time)
    console.log(`\n${colors.bright}${colors.cyan}???????????????????????????????????????????${colors.reset}`);
    console.log(`${colors.bright}${colors.cyan}   TEST 8: JobType Filter (Full-Time)${colors.reset}`);
    console.log(`${colors.bright}${colors.cyan}???????????????????????????????????????????${colors.reset}\n`);
    
    const { whereClause: where8, params: params8 } = buildJobFilters({ jobTypeIds: '1' }, userId);
    const jobTypeQuery8 = `
        SELECT COUNT(*) as total
        FROM Jobs j
        INNER JOIN Organizations o ON j.OrganizationID = o.OrganizationID
        ${where8}
    `;
    results.push(await executeQueryWithTiming(pool, 'JobType: Full-Time (JobTypeID=1)', jobTypeQuery8, params8));
    
    // Test 9: Location Filter (Bangalore)
    console.log(`\n${colors.bright}${colors.cyan}???????????????????????????????????????????${colors.reset}`);
    console.log(`${colors.bright}${colors.cyan}   TEST 9: Location Filter (Bangalore)${colors.reset}`);
    console.log(`${colors.bright}${colors.cyan}???????????????????????????????????????????${colors.reset}\n`);
    
    const { whereClause: where9, params: params9 } = buildJobFilters({ location: 'Bangalore' }, userId);
    const locationQuery9 = `
        SELECT COUNT(*) as total
        FROM Jobs j
        INNER JOIN Organizations o ON j.OrganizationID = o.OrganizationID
        ${where9}
    `;
    results.push(await executeQueryWithTiming(pool, 'Location: Bangalore', locationQuery9, params9));
    
    // Test 10: Company/Organization Filter
    console.log(`\n${colors.bright}${colors.cyan}???????????????????????????????????????????${colors.reset}`);
    console.log(`${colors.bright}${colors.cyan}   TEST 10: Company Filter (Org 1,2,3)${colors.reset}`);
    console.log(`${colors.bright}${colors.cyan}???????????????????????????????????????????${colors.reset}\n`);
    
    const { whereClause: where10, params: params10 } = buildJobFilters({ organizationIds: '1,2,3' }, userId);
    const companyQuery10 = `
        SELECT COUNT(*) as total
        FROM Jobs j
        INNER JOIN Organizations o ON j.OrganizationID = o.OrganizationID
        ${where10}
    `;
    results.push(await executeQueryWithTiming(pool, 'Company: OrganizationID IN (1,2,3)', companyQuery10, params10));
    
    // Test 11: Experience Filter (2-5 years)
    console.log(`\n${colors.bright}${colors.cyan}???????????????????????????????????????????${colors.reset}`);
    console.log(`${colors.bright}${colors.cyan}   TEST 11: Experience Filter (2-5 years)${colors.reset}`);
    console.log(`${colors.bright}${colors.cyan}???????????????????????????????????????????${colors.reset}\n`);
    
    const { whereClause: where11, params: params11 } = buildJobFilters({ 
        experienceMin: 2, 
        experienceMax: 5 
    }, userId);
    const experienceQuery11 = `
        SELECT COUNT(*) as total
        FROM Jobs j
        INNER JOIN Organizations o ON j.OrganizationID = o.OrganizationID
        ${where11}
    `;
    results.push(await executeQueryWithTiming(pool, 'Experience: 2-5 years', experienceQuery11, params11));
    
    // Test 12: Salary Filter (50k-100k)
    console.log(`\n${colors.bright}${colors.cyan}???????????????????????????????????????????${colors.reset}`);
    console.log(`${colors.bright}${colors.cyan}   TEST 12: Salary Filter (50k-100k)${colors.reset}`);
    console.log(`${colors.bright}${colors.cyan}???????????????????????????????????????????${colors.reset}\n`);
    
    const { whereClause: where12, params: params12 } = buildJobFilters({ 
        salaryMin: 50000, 
        salaryMax: 100000 
    }, userId);
    const salaryQuery12 = `
        ...
        ${where12}
    `;
    results.push(await executeQueryWithTiming(pool, 'Salary: 50k-100k', salaryQuery12, params12));
    
    // Test 13: Freshness Filter (Last 24 hours)
    console.log(`\n${colors.bright}${colors.cyan}???????????????????????????????????????????${colors.reset}`);
    console.log(`${colors.bright}${colors.cyan}   TEST 13: Freshness Filter (24 hours)${colors.reset}`);
    console.log(`${colors.bright}${colors.cyan}???????????????????????????????????????????${colors.reset}\n`);
    
    const { whereClause: where13, params: params13 } = buildJobFilters({ postedWithinDays: 1 }, userId);
    const freshnessQuery13 = `
        SELECT COUNT(*) as total
        FROM Jobs j
        INNER JOIN Organizations o ON j.OrganizationID = o.OrganizationID
        ${where13}
    `;
    results.push(await executeQueryWithTiming(pool, 'Freshness: Last 24 hours', freshnessQuery13, params13));
    
    // Test 14: Combined Popular Filters (JobType + Workplace + Location)
    console.log(`\n${colors.bright}${colors.cyan}???????????????????????????????????????????${colors.reset}`);
    console.log(`${colors.bright}${colors.cyan}   TEST 14: Combined Filters (Full-Time + Remote + Bangalore)${colors.reset}`);
    console.log(`${colors.bright}${colors.cyan}???????????????????????????????????????????${colors.reset}\n`);
    
    const { whereClause: where14, params: params14 } = buildJobFilters({ 
        jobTypeIds: '1',
        workplaceTypeIds: '3',
        location: 'Bangalore'
    }, userId);
    const combinedQuery14 = `
        SELECT COUNT(*) as total
        FROM Jobs j
        INNER JOIN Organizations o ON j.OrganizationID = o.OrganizationID
        ${where14}
    `;
    results.push(await executeQueryWithTiming(pool, 'Combined: Full-Time + Remote + Bangalore', combinedQuery14, params14));
    
    // Test 15: All Frontend Filters Combined (Worst Case)
    console.log(`\n${colors.bright}${colors.cyan}???????????????????????????????????????????${colors.reset}`);
    console.log(`${colors.bright}${colors.cyan}   TEST 15: All Filters Combined (Worst Case)${colors.reset}`);
    console.log(`${colors.bright}${colors.cyan}???????????????????????????????????????????${colors.reset}\n`);
    
    const { whereClause: where15, params: params15 } = buildJobFilters({ 
        jobTypeIds: '1,2',
        workplaceTypeIds: '3',
        location: 'Bangalore',
    ...
    const allFiltersQuery15 = `
        SELECT COUNT(*) as total
        FROM Jobs j
        INNER JOIN Organizations o ON j.OrganizationID = o.OrganizationID
        ${where15}
    `;
    results.push(await executeQueryWithTiming(pool, 'ALL Filters Combined', allFiltersQuery15, params15));
